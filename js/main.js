...
function applyRadialLayoutBySum(sprites, finalScale){
  ...
  groups.forEach((list,sum)=>{
    ...
    const r=Math.max(0, r_in_level - spriteR*RAD_MARGIN_COEFF);
    const z=getSumZ(sum);
    if(list.length===1){
      if(t===1){
...
function applyDuplicateRingLayout(sprites, finalScale){
  ...
  const baseRingStep=spriteR*1.25;                    // リング間距離基準
  const MAX_PER_RING=12;                              // 1リング最大
  const ANGLE_OFFSET_PER_RING=0.18;                   // リング毎回転
  const JITTER=spriteR*0.07;                          // 微揺らぎ
  /* 三角形辺距離計算用（2D） */
  const tri=[A,B,C];
  ...
  groups.forEach(list=>{
    ...
    for(let k=0;k<dupCount;k++){
      ...
      /* 希望半径 */
      let desired=baseRingStep*(1 + ringLevel*1.10);
...
function rebuild(){
  try{
    ...
    collisionScaleVal.textContent='col '+collisionExpand.toFixed(3)+(collisionExpand>=2.999?'*':'');
